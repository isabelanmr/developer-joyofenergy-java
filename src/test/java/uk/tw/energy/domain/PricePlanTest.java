...
public class PricePlanTest {
    ...
    //        Instant peakTime = LocalDateTime.of(2024, Month.JULY, 18, 18, 30).toInstant(ZoneOffset.UTC); // Horário de
    // pico
    //        Instant offPeakTime = LocalDateTime.of(2024, Month.JULY, 18, 10, 0).toInstant(ZoneOffset.UTC); // Horário
    // fora de pico
    //
    ...
    //
    //        Optional<Map<String, BigDecimal>> costMap =
    // pricePlanService.getConsumptionCostOfElectricityReadingsForEachPricePlan("smart-meter-0");
    //
    //        assertThat(costMap).isPresent();
    //        BigDecimal cost = costMap.get().get("price-plan-0");
    //
    //        // Calculando o custo esperado
    //        BigDecimal expectedCost = BigDecimal.valueOf((0.5 * 2 + 0.5) /
    // 2).multiply(BigDecimal.TEN).divide(BigDecimal.valueOf(10.0 / 3600.0), RoundingMode.HALF_UP);
    //
